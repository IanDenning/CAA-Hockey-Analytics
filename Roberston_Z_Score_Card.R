...
Wingers_Z_Cards <- Forwards_zScores_Master

# ============================================================
# PLAYER CONFIG — only edit this block per player
# ============================================================

player_name     <- "Robertson, Jason"
player_team     <- "DAL"
player_pos      <- "LW"
player_number   <- 21
player_age      <- 26
...
player_weight   <- 205
player_aav      <- "$7.750M"
player_contract <- "SPC expires 2026 (RFA)"
headshot_path   <- "/Users/iandenning/Downloads/Roberston_Jason.png"

# Flex slot 1 — deployment context (same as centers)
...
# Flex slot 2 — WINGER: individual high-danger chances (replaces faceoffs)
flex2_col          <- "iHDCF_per60_percentile"
flex2_label        <- "Ind. HD Chances/60"
flex2_raw_col      <- "iHDCF_per60"
flex2_raw_fmt      <- function(x) as.character(round(x, 2))
...
p_row <- Wingers_Z_Cards %>% filter(Player == player_name)
